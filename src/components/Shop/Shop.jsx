import React, { useState, useEffect, useContext } from 'react'
import shopStyles from './Shop.module.css'
import Container from 'react-bootstrap/Container';
import Row from 'react-bootstrap/Row';
import Col from 'react-bootstrap/Col';
import { Link, useLocation } from 'react-router-dom';
import Accordion from 'react-bootstrap/Accordion';
import RangeSlider from 'react-range-slider-input';
import 'react-range-slider-input/dist/style.css';
import Card from 'react-bootstrap/Card'
import Image from 'react-bootstrap/Image'
import Form from 'react-bootstrap/Form'
import Offcanvas from 'react-bootstrap/Offcanvas';
import { ProductContext } from '../../context/ProductContext';
import { ProductMoreContext } from '../../context/ProductMoreContext';


const Shop = () => {

    const location = useLocation();
    const pathSegments = location.pathname.split('/').filter(Boolean);

    let currentPath = '';
    const crumbs = pathSegments.map((segment, index) => {
        currentPath += `/${segment}`;
        const isLast = index === pathSegments.length - 1;

        return (
            <span key={currentPath}>
                {isLast ? (
                    <span>{segment.charAt(0).toUpperCase() + segment.slice(1)}</span>
                ) : (
                    <>
                        <Link to={currentPath}>{segment}</Link>
                        <span> / </span>
                    </>
                )}
            </span>
        );
    });

    const categoryItems = [
        {
            id: 1,
            title: 'T-shirts',
        },
        {
            id: 2,
            title: 'Shorts',
        },
        {
            id: 3,
            title: 'Shirts',
        },
        {
            id: 4,
            title: 'Hoodie',
        },
        {
            id: 5,
            title: 'Jeans',
        },
    ]

    const [value, setValue] = useState([20, 80])

    const [products, setProducts] = useState([])

    const [resultPageNum, setResultPageNum] = useState(1)

    const nextButtons = Array.from({ length: 5 }, (_, index) => index).filter((value) => value > 0)

    console.log(resultPageNum);

    const { productMore, setProductMore } = useContext(ProductMoreContext)

    const fetchData = async () => {
        try {
            const response = await fetch(`https://fakestoreapiserver.reactbd.org/api/products?page=${resultPageNum}&perPage=9`)
            const data = await response.json()
            console.log(data)
            setProducts(data.data)
            setProductMore(data.data)
            localStorage.setItem('productMore', JSON.stringify(data.data));
        } catch (error) {
            console.error('Error:', error)
        }
    }

    useEffect(() => {
        fetchData()
    }, [resultPageNum])

    console.log(products);

    console.log(productMore);

    const colorVariations = [
        {
            id: 1,
            color: '#00C12B',
        },
        {
            id: 2,
            color: '#F50606',
        },
        {
            id: 3,
            color: '#F5DD06',
        },
        {
            id: 4,
            color: '#F57906',
        },
        {
            id: 5,
            color: '#06CAF5',
        },
        {
            id: 6,
            color: '#063AF5',
        },
        {
            id: 7,
            color: '#7D06F5',
        },
        {
            id: 8,
            color: '#F506A4',
        },
        {
            id: 9,
            color: '#FFFFFF',
        },
        {
            id: 10,
            color: '#000000',
        },
    ]

    const sizeVariations = [
        {
            id: 1,
            size: 'XX-Small',
        },
        {
            id: 2,
            size: 'X-Small',
        },
        {
            id: 3,
            size: 'Small',
        },
        {
            id: 4,
            size: 'Medium',
        },
        {
            id: 5,
            size: 'Large',
        },
        {
            id: 6,
            size: 'X-Large',
        },
        {
            id: 7,
            size: 'XX-Large',
        },
        {
            id: 8,
            size: '3X-Large',
        },
        {
            id: 9,
            size: '4X-Large',
        },

    ]

    const [sizebtn, setSizeBtn] = useState(0)

    const dressStyle = [
        {
            id: 1,
            style: 'Casual'
        },
        {
            id: 2,
            style: 'Formal'
        },
        {
            id: 3,
            style: 'Party'
        },
        {
            id: 4,
            style: 'Gym'
        },
    ]

    const [show, setShow] = useState(false);

    const handleClose = () => setShow(false);
    const handleShow = () => setShow(true);

    const { productID, setProductID } = useContext(ProductContext)


    console.log(productID);


    return (
        <section className={shopStyles.shopcontainer}>
            <Container fluid className='py-lg-5 py-3'>
                <Row>
                    <Col lg={12} className='border-top border-tertiary py-lg-4 py-3'>
                        <nav aria-label="breadcrumb">
                            <ol className="breadcrumb" style={{ fontSize: '14px' }}>
                                <li className="breadcrumb-item me-2">
                                    <Link to="/" className='text-decoration-none text-muted'>Home</Link>
                                </li>
                                <span className='me-2'>
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M6.53073 2.46937L11.5307 7.46937C11.6007 7.53905 11.6561 7.62184 11.694 7.71301C11.7318 7.80417 11.7513 7.90191 11.7513 8.00062C11.7513 8.09933 11.7318 8.19707 11.694 8.28824C11.6561 8.3794 11.6007 8.46219 11.5307 8.53187L6.53073 13.5319C6.38984 13.6728 6.19874 13.7519 5.99948 13.7519C5.80023 13.7519 5.60913 13.6728 5.46823 13.5319C5.32734 13.391 5.24818 13.1999 5.24818 13.0006C5.24818 12.8014 5.32734 12.6103 5.46823 12.4694L9.93761 8L5.46761 3.53062C5.32671 3.38973 5.24756 3.19863 5.24756 2.99937C5.24756 2.80011 5.32671 2.60902 5.46761 2.46812C5.60851 2.32723 5.7996 2.24807 5.99886 2.24807C6.19812 2.24807 6.38921 2.32723 6.53011 2.46812L6.53073 2.46937Z" fill="black" fill-opacity="0.6" />
                                    </svg>
                                </span>
                                {crumbs.length > 0 && crumbs}
                            </ol>
                        </nav>
                    </Col>
                    <Col lg={3} md={5} className='d-lg-block d-md-block d-none'>
                        <div className={shopStyles.filterscontainer}>
                            <div className='d-flex justify-content-between border-bottom border-tertiary pb-3'>
                                <h5 className='fw-semibold'>Filters</h5>
                                <span>
                                    <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M13.125 12.125V20.75C13.125 21.0484 13.0065 21.3345 12.7955 21.5455C12.5845 21.7565 12.2984 21.875 12 21.875C11.7016 21.875 11.4155 21.7565 11.2045 21.5455C10.9935 21.3345 10.875 21.0484 10.875 20.75V12.125C10.875 11.8266 10.9935 11.5405 11.2045 11.3295C11.4155 11.1185 11.7016 11 12 11C12.2984 11 12.5845 11.1185 12.7955 11.3295C13.0065 11.5405 13.125 11.8266 13.125 12.125ZM18.75 18.5C18.4516 18.5 18.1655 18.6185 17.9545 18.8295C17.7435 19.0405 17.625 19.3266 17.625 19.625V20.75C17.625 21.0484 17.7435 21.3345 17.9545 21.5455C18.1655 21.7565 18.4516 21.875 18.75 21.875C19.0484 21.875 19.3345 21.7565 19.5455 21.5455C19.7565 21.3345 19.875 21.0484 19.875 20.75V19.625C19.875 19.3266 19.7565 19.0405 19.5455 18.8295C19.3345 18.6185 19.0484 18.5 18.75 18.5ZM21 14.75H19.875V4.25C19.875 3.95163 19.7565 3.66548 19.5455 3.4545C19.3345 3.24353 19.0484 3.125 18.75 3.125C18.4516 3.125 18.1655 3.24353 17.9545 3.4545C17.7435 3.66548 17.625 3.95163 17.625 4.25V14.75H16.5C16.2016 14.75 15.9155 14.8685 15.7045 15.0795C15.4935 15.2905 15.375 15.5766 15.375 15.875C15.375 16.1734 15.4935 16.4595 15.7045 16.6705C15.9155 16.8815 16.2016 17 16.5 17H21C21.2984 17 21.5845 16.8815 21.7955 16.6705C22.0065 16.4595 22.125 16.1734 22.125 15.875C22.125 15.5766 22.0065 15.2905 21.7955 15.0795C21.5845 14.8685 21.2984 14.75 21 14.75ZM5.25 15.5C4.95163 15.5 4.66548 15.6185 4.4545 15.8295C4.24353 16.0405 4.125 16.3266 4.125 16.625V20.75C4.125 21.0484 4.24353 21.3345 4.4545 21.5455C4.66548 21.7565 4.95163 21.875 5.25 21.875C5.54837 21.875 5.83452 21.7565 6.0455 21.5455C6.25647 21.3345 6.375 21.0484 6.375 20.75V16.625C6.375 16.3266 6.25647 16.0405 6.0455 15.8295C5.83452 15.6185 5.54837 15.5 5.25 15.5ZM7.5 11.75H6.375V4.25C6.375 3.95163 6.25647 3.66548 6.0455 3.4545C5.83452 3.24353 5.54837 3.125 5.25 3.125C4.95163 3.125 4.66548 3.24353 4.4545 3.4545C4.24353 3.66548 4.125 3.95163 4.125 4.25V11.75H3C2.70163 11.75 2.41548 11.8685 2.2045 12.0795C1.99353 12.2905 1.875 12.5766 1.875 12.875C1.875 13.1734 1.99353 13.4595 2.2045 13.6705C2.41548 13.8815 2.70163 14 3 14H7.5C7.79837 14 8.08452 13.8815 8.2955 13.6705C8.50647 13.4595 8.625 13.1734 8.625 12.875C8.625 12.5766 8.50647 12.2905 8.2955 12.0795C8.08452 11.8685 7.79837 11.75 7.5 11.75ZM14.25 7.25H13.125V4.25C13.125 3.95163 13.0065 3.66548 12.7955 3.4545C12.5845 3.24353 12.2984 3.125 12 3.125C11.7016 3.125 11.4155 3.24353 11.2045 3.4545C10.9935 3.66548 10.875 3.95163 10.875 4.25V7.25H9.75C9.45163 7.25 9.16548 7.36853 8.9545 7.5795C8.74353 7.79048 8.625 8.07663 8.625 8.375C8.625 8.67337 8.74353 8.95952 8.9545 9.1705C9.16548 9.38147 9.45163 9.5 9.75 9.5H14.25C14.5484 9.5 14.8345 9.38147 15.0455 9.1705C15.2565 8.95952 15.375 8.67337 15.375 8.375C15.375 8.07663 15.2565 7.79048 15.0455 7.5795C14.8345 7.36853 14.5484 7.25 14.25 7.25Z" fill="black" fill-opacity="0.4" />
                                    </svg>
                                </span>
                            </div>
                            <div className='pt-4 pb-3 border-bottom border-tertiary'>
                                {categoryItems.map((item, index) =>
                                    <ul className='list-unstyled' key={index}>
                                        <div className='d-flex justify-content-between'>
                                            <li><a href="#" className='text-decoration-none text-muted'>{item.title}</a></li>
                                            <span><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M6.53073 2.4694L11.5307 7.4694C11.6007 7.53908 11.6561 7.62187 11.694 7.71304C11.7318 7.8042 11.7513 7.90194 11.7513 8.00065C11.7513 8.09936 11.7318 8.1971 11.694 8.28827C11.6561 8.37943 11.6007 8.46222 11.5307 8.5319L6.53073 13.5319C6.38984 13.6728 6.19874 13.752 5.99948 13.752C5.80023 13.752 5.60913 13.6728 5.46823 13.5319C5.32734 13.391 5.24818 13.1999 5.24818 13.0007C5.24818 12.8014 5.32734 12.6103 5.46823 12.4694L9.93761 8.00003L5.46761 3.53065C5.32671 3.38976 5.24756 3.19866 5.24756 2.9994C5.24756 2.80015 5.32671 2.60905 5.46761 2.46815C5.60851 2.32726 5.7996 2.2481 5.99886 2.2481C6.19812 2.2481 6.38921 2.32726 6.53011 2.46815L6.53073 2.4694Z" fill="black" fill-opacity="0.6" />
                                            </svg>
                                            </span>
                                        </div>
                                    </ul>
                                )}
                            </div>
                            <div className='pt-4 pb-3 border-bottom border-tertiary'>
                                <Accordion defaultActiveKey="0" id='price-range-slider'>
                                    <Accordion.Item eventKey="0" className='border-0'>
                                        <Accordion.Header><h5 className='fw-semibold'>Price</h5></Accordion.Header>
                                        <Accordion.Body className='py-3 px-0'>
                                            <div>
                                                <RangeSlider
                                                    min={0}
                                                    max={100}
                                                    value={value}
                                                    onInput={setValue}
                                                    id="range-slider-black"
                                                />
                                                <div className='d-flex justify-content-around'>
                                                    <span className={`${shopStyles.pricerangelabel} d-block mt-3`}>${value[0]}</span>
                                                    <span className={`${shopStyles.pricerangelabel} d-block mt-3`}>${value[1]}</span>
                                                </div>
                                            </div>
                                        </Accordion.Body>
                                    </Accordion.Item>
                                </Accordion>
                            </div>
                            <div className='pt-4 pb-3 border-bottom border-tertiary'>
                                <Accordion defaultActiveKey="0" id='color-picker'>
                                    <Accordion.Item eventKey="0" className='border-0'>
                                        <Accordion.Header><h5 className='fw-semibold'>Colors</h5></Accordion.Header>
                                        <Accordion.Body className='py-3 px-0'>
                                            <div class={shopStyles['color-picker']}>
                                                {colorVariations.map((item, index) => <div key={index}>
                                                    <input type="radio" id={item.color} name="color" value={item.color} />
                                                    <label for={item.color} style={{ backgroundColor: item.color, border: '1px solid rgba(0,0,0,0.2)' }}></label>
                                                </div>)}
                                            </div>
                                        </Accordion.Body>
                                    </Accordion.Item>
                                </Accordion>
                            </div>
                            <div className='pt-4 pb-3 border-bottom border-tertiary'>
                                <Accordion defaultActiveKey="0" id='size-options'>
                                    <Accordion.Item eventKey="0" className='border-0'>
                                        <Accordion.Header><h5 className='fw-semibold'>Size</h5></Accordion.Header>
                                        <Accordion.Body className='py-3 px-0'>
                                            <div className={shopStyles.sizebtncontainer}>
                                                {sizeVariations.map((item, index) => <div key={index}>
                                                    <button className={sizebtn === index ? shopStyles.activesizebtn : shopStyles.sizebtn} onClick={() => setSizeBtn(index)}>{item.size}</button>
                                                </div>)}
                                            </div>
                                        </Accordion.Body>
                                    </Accordion.Item>
                                </Accordion>
                            </div>
                            <div className='pt-4 pb-3'>
                                <Accordion defaultActiveKey="0" id='dress-style'>
                                    <Accordion.Item eventKey="0" className='border-0'>
                                        <Accordion.Header><h5 className='fw-semibold'>Dress Style</h5></Accordion.Header>
                                        <Accordion.Body className='py-3 px-0'>
                                            {dressStyle.map((item, index) =>
                                                <ul className='list-unstyled' key={index}>
                                                    <div className='d-flex justify-content-between'>
                                                        <li><a href="#" className='text-decoration-none text-muted'>{item.style}</a></li>
                                                        <span><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M6.53073 2.4694L11.5307 7.4694C11.6007 7.53908 11.6561 7.62187 11.694 7.71304C11.7318 7.8042 11.7513 7.90194 11.7513 8.00065C11.7513 8.09936 11.7318 8.1971 11.694 8.28827C11.6561 8.37943 11.6007 8.46222 11.5307 8.5319L6.53073 13.5319C6.38984 13.6728 6.19874 13.752 5.99948 13.752C5.80023 13.752 5.60913 13.6728 5.46823 13.5319C5.32734 13.391 5.24818 13.1999 5.24818 13.0007C5.24818 12.8014 5.32734 12.6103 5.46823 12.4694L9.93761 8.00003L5.46761 3.53065C5.32671 3.38976 5.24756 3.19866 5.24756 2.9994C5.24756 2.80015 5.32671 2.60905 5.46761 2.46815C5.60851 2.32726 5.7996 2.2481 5.99886 2.2481C6.19812 2.2481 6.38921 2.32726 6.53011 2.46815L6.53073 2.4694Z" fill="black" fill-opacity="0.6" />
                                                        </svg>
                                                        </span>
                                                    </div>
                                                </ul>
                                            )}
                                        </Accordion.Body>
                                    </Accordion.Item>
                                </Accordion>
                            </div>
                            <div>
                                <button className={`${shopStyles.applyfilterbtn} d-block w-100`}>Apply Filter</button>
                            </div>
                        </div>
                    </Col>
                    <Col lg={9} md={7}>
                        <div className='mb-4 d-flex align-items-lg-center justify-content-between flex-lg-row flex-column'>
                            <div className='d-flex justify-content-between'>
                                <h2 className='fw-semibold'>Casual</h2>
                                <div className={`${shopStyles.filtermenuiconbg} d-lg-none d-md-none d-flex`}>
                                    <svg onClick={handleShow} width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M8.75 7.75V13.5C8.75 13.6989 8.67098 13.8897 8.53033 14.0303C8.38968 14.171 8.19891 14.25 8 14.25C7.80109 14.25 7.61032 14.171 7.46967 14.0303C7.32902 13.8897 7.25 13.6989 7.25 13.5V7.75C7.25 7.55109 7.32902 7.36032 7.46967 7.21967C7.61032 7.07902 7.80109 7 8 7C8.19891 7 8.38968 7.07902 8.53033 7.21967C8.67098 7.36032 8.75 7.55109 8.75 7.75ZM12.5 12C12.3011 12 12.1103 12.079 11.9697 12.2197C11.829 12.3603 11.75 12.5511 11.75 12.75V13.5C11.75 13.6989 11.829 13.8897 11.9697 14.0303C12.1103 14.171 12.3011 14.25 12.5 14.25C12.6989 14.25 12.8897 14.171 13.0303 14.0303C13.171 13.8897 13.25 13.6989 13.25 13.5V12.75C13.25 12.5511 13.171 12.3603 13.0303 12.2197C12.8897 12.079 12.6989 12 12.5 12ZM14 9.5H13.25V2.5C13.25 2.30109 13.171 2.11032 13.0303 1.96967C12.8897 1.82902 12.6989 1.75 12.5 1.75C12.3011 1.75 12.1103 1.82902 11.9697 1.96967C11.829 2.11032 11.75 2.30109 11.75 2.5V9.5H11C10.8011 9.5 10.6103 9.57902 10.4697 9.71967C10.329 9.86032 10.25 10.0511 10.25 10.25C10.25 10.4489 10.329 10.6397 10.4697 10.7803C10.6103 10.921 10.8011 11 11 11H14C14.1989 11 14.3897 10.921 14.5303 10.7803C14.671 10.6397 14.75 10.4489 14.75 10.25C14.75 10.0511 14.671 9.86032 14.5303 9.71967C14.3897 9.57902 14.1989 9.5 14 9.5ZM3.5 10C3.30109 10 3.11032 10.079 2.96967 10.2197C2.82902 10.3603 2.75 10.5511 2.75 10.75V13.5C2.75 13.6989 2.82902 13.8897 2.96967 14.0303C3.11032 14.171 3.30109 14.25 3.5 14.25C3.69891 14.25 3.88968 14.171 4.03033 14.0303C4.17098 13.8897 4.25 13.6989 4.25 13.5V10.75C4.25 10.5511 4.17098 10.3603 4.03033 10.2197C3.88968 10.079 3.69891 10 3.5 10ZM5 7.5H4.25V2.5C4.25 2.30109 4.17098 2.11032 4.03033 1.96967C3.88968 1.82902 3.69891 1.75 3.5 1.75C3.30109 1.75 3.11032 1.82902 2.96967 1.96967C2.82902 2.11032 2.75 2.30109 2.75 2.5V7.5H2C1.80109 7.5 1.61032 7.57902 1.46967 7.71967C1.32902 7.86032 1.25 8.05109 1.25 8.25C1.25 8.44891 1.32902 8.63968 1.46967 8.78033C1.61032 8.92098 1.80109 9 2 9H5C5.19891 9 5.38968 8.92098 5.53033 8.78033C5.67098 8.63968 5.75 8.44891 5.75 8.25C5.75 8.05109 5.67098 7.86032 5.53033 7.71967C5.38968 7.57902 5.19891 7.5 5 7.5ZM9.5 4.5H8.75V2.5C8.75 2.30109 8.67098 2.11032 8.53033 1.96967C8.38968 1.82902 8.19891 1.75 8 1.75C7.80109 1.75 7.61032 1.82902 7.46967 1.96967C7.32902 2.11032 7.25 2.30109 7.25 2.5V4.5H6.5C6.30109 4.5 6.11032 4.57902 5.96967 4.71967C5.82902 4.86032 5.75 5.05109 5.75 5.25C5.75 5.44891 5.82902 5.63968 5.96967 5.78033C6.11032 5.92098 6.30109 6 6.5 6H9.5C9.69891 6 9.88968 5.92098 10.0303 5.78033C10.171 5.63968 10.25 5.44891 10.25 5.25C10.25 5.05109 10.171 4.86032 10.0303 4.71967C9.88968 4.57902 9.69891 4.5 9.5 4.5Z" fill="black" />
                                    </svg>
                                </div>
                            </div>
                            <div className='d-flex align-items-center'>
                                <span style={{ fontSize: '14px', color: 'rgba(0,0,0,0.6)' }} className='me-3'>Showing 1-4 of 30 Products</span>
                                <span style={{ fontSize: '12px' }} className='d-flex align-items-center'>Sort by:
                                    <Form.Select aria-label="Default select example" className='w-75 border-0' style={{ fontSize: '13px' }}>
                                        <option>Most Popular</option>
                                        <option value="1">High Price</option>
                                        <option value="2">Low Price</option>
                                    </Form.Select>
                                </span>
                            </div>
                        </div>
                        <Row className='border-bottom border-tertiary pb-4'>
                            {products.map((item, index) =>
                                <Col lg={4} md={6} xs={6} key={index} className={shopStyles.prodresults} onClick={() => { setProductID(item._id); localStorage.setItem('productID', item._id) }}>
                                    <Link to='/shop/product' className='text-decoration-none'>
                                        <Card className='border-0'>
                                            <Card.Body className='p-0'>
                                                <div className='mb-3'>
                                                    <Image src={item.image} fluid className={shopStyles.productimg} />
                                                </div>
                                                <Card.Title className='fw-semibold text-capitalize'>{item.title}</Card.Title>
                                                <div className='d-flex align-items-center pb-2'>
                                                    {[1, 2, 3, 4].map((_, index) => <div className='d-flex' key={index}><svg width="19" height="17" viewBox="0 0 19 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M9.24494 0.255066L11.8641 5.89498L18.0374 6.64316L13.4829 10.877L14.679 16.9793L9.24494 13.9561L3.8109 16.9793L5.00697 10.877L0.452479 6.64316L6.62573 5.89498L9.24494 0.255066Z" fill="#FFC633" />
                                                    </svg>
                                                    </div>)}
                                                    <div className={`${shopStyles.ratingtext} ms-2 pt-1`}>{item.rating}/5</div>
                                                </div>
                                                <div className='d-flex align-items-center pb-4'>
                                                    <div>
                                                        <span className={`${shopStyles.sliderprice} me-2 fw-semibold`}>${item.discountedPrice}</span>
                                                    </div>
                                                    <div>
                                                        <span className={`${shopStyles.slideroldprice} me-2 fw-semibold`}>${item.oldPrice}</span>
                                                    </div>
                                                    <div className={shopStyles.sliderdiscountbg}>
                                                        <span className={shopStyles.sliderdiscounttext}>{100 - Math.round(item.discountedPrice / item.oldPrice * 100)}%</span>
                                                    </div>
                                                </div>
                                            </Card.Body>
                                        </Card>
                                    </Link>
                                </Col>)}
                        </Row>
                        <div className='d-flex justify-content-between align-items-center py-4'>
                            <div>
                                <button className={shopStyles.paginatebtn} onClick={() => setResultPageNum(resultPageNum > 0 ? resultPageNum - 1 : resultPageNum)}><span className='me-2'><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M15.8332 9.99996H4.1665M4.1665 9.99996L9.99984 15.8333M4.1665 9.99996L9.99984 4.16663" stroke="black" stroke-width="1.67" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                                </span> Previous
                                </button>
                            </div>
                            <div>
                                {nextButtons.map((item, index) => { return (<button key={index} className={`${resultPageNum === item ? shopStyles.activenumbtn : shopStyles.numbtn} me-2`} onClick={() => setResultPageNum(item)}>{item}</button>) })}
                            </div>
                            <div>
                                <button className={shopStyles.paginatebtn} onClick={() => setResultPageNum(resultPageNum < 4 ? resultPageNum + 1 : resultPageNum)}>Next<span className='ms-2'><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M4.1665 9.99996H15.8332M15.8332 9.99996L9.99984 4.16663M15.8332 9.99996L9.99984 15.8333" stroke="black" stroke-width="1.67" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                                </span>
                                </button>
                            </div>
                        </div>

                    </Col>
                    <Offcanvas show={show} onHide={handleClose} className={`${shopStyles.filtermenu} w-100 h-75`} placement='bottom'>
                        <Offcanvas.Body>
                            <div>
                                <div className='d-flex justify-content-between border-bottom border-tertiary pb-3'>
                                    <h5 className='fw-semibold'>Filters</h5>
                                    <span onClick={handleClose}>
                                        <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M19.5459 18.4541C19.7572 18.6654 19.876 18.952 19.876 19.2509C19.876 19.5498 19.7572 19.8365 19.5459 20.0478C19.3346 20.2592 19.0479 20.3779 18.749 20.3779C18.4501 20.3779 18.1635 20.2592 17.9521 20.0478L12 14.0937L6.0459 20.0459C5.83455 20.2573 5.54791 20.376 5.24902 20.376C4.95014 20.376 4.66349 20.2573 4.45215 20.0459C4.2408 19.8346 4.12207 19.5479 4.12207 19.2491C4.12207 18.9502 4.2408 18.6635 4.45215 18.4522L10.4062 12.5L4.45402 6.54593C4.24268 6.33459 4.12395 6.04795 4.12395 5.74906C4.12395 5.45017 4.24268 5.16353 4.45402 4.95218C4.66537 4.74084 4.95201 4.62211 5.2509 4.62211C5.54978 4.62211 5.83643 4.74084 6.04777 4.95218L12 10.9062L17.954 4.95125C18.1654 4.7399 18.452 4.62117 18.7509 4.62117C19.0498 4.62117 19.3364 4.7399 19.5478 4.95125C19.7591 5.16259 19.8778 5.44924 19.8778 5.74812C19.8778 6.04701 19.7591 6.33365 19.5478 6.545L13.5937 12.5L19.5459 18.4541Z" fill="black" fill-opacity="0.4" />
                                        </svg>
                                    </span>
                                </div>
                                <div className='pt-4 pb-3 border-bottom border-tertiary'>
                                    {categoryItems.map((item, index) =>
                                        <ul className='list-unstyled' key={index}>
                                            <div className='d-flex justify-content-between'>
                                                <li><a href="#" className='text-decoration-none text-muted'>{item.title}</a></li>
                                                <span><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M6.53073 2.4694L11.5307 7.4694C11.6007 7.53908 11.6561 7.62187 11.694 7.71304C11.7318 7.8042 11.7513 7.90194 11.7513 8.00065C11.7513 8.09936 11.7318 8.1971 11.694 8.28827C11.6561 8.37943 11.6007 8.46222 11.5307 8.5319L6.53073 13.5319C6.38984 13.6728 6.19874 13.752 5.99948 13.752C5.80023 13.752 5.60913 13.6728 5.46823 13.5319C5.32734 13.391 5.24818 13.1999 5.24818 13.0007C5.24818 12.8014 5.32734 12.6103 5.46823 12.4694L9.93761 8.00003L5.46761 3.53065C5.32671 3.38976 5.24756 3.19866 5.24756 2.9994C5.24756 2.80015 5.32671 2.60905 5.46761 2.46815C5.60851 2.32726 5.7996 2.2481 5.99886 2.2481C6.19812 2.2481 6.38921 2.32726 6.53011 2.46815L6.53073 2.4694Z" fill="black" fill-opacity="0.6" />
                                                </svg>
                                                </span>
                                            </div>
                                        </ul>
                                    )}
                                </div>
                                <div className='pt-4 pb-3 border-bottom border-tertiary'>
                                    <Accordion defaultActiveKey="0" id='price-range-slider'>
                                        <Accordion.Item eventKey="0" className='border-0'>
                                            <Accordion.Header><h5 className='fw-semibold'>Price</h5></Accordion.Header>
                                            <Accordion.Body className='py-3 px-0'>
                                                <div>
                                                    <RangeSlider
                                                        min={0}
                                                        max={100}
                                                        value={value}
                                                        onInput={setValue}
                                                        id="range-slider-black"
                                                    />
                                                    <div className='d-flex justify-content-around'>
                                                        <span className={`${shopStyles.pricerangelabel} d-block mt-3`}>${value[0]}</span>
                                                        <span className={`${shopStyles.pricerangelabel} d-block mt-3`}>${value[1]}</span>
                                                    </div>
                                                </div>
                                            </Accordion.Body>
                                        </Accordion.Item>
                                    </Accordion>
                                </div>
                                <div className='pt-4 pb-3 border-bottom border-tertiary'>
                                    <Accordion defaultActiveKey="0" id='color-picker'>
                                        <Accordion.Item eventKey="0" className='border-0'>
                                            <Accordion.Header><h5 className='fw-semibold'>Colors</h5></Accordion.Header>
                                            <Accordion.Body className='py-3 px-0'>
                                                <div class={shopStyles['color-picker']}>
                                                    {colorVariations.map((item, index) => <div key={index}>
                                                        <input type="radio" id={item.color} name="color" value={item.color} />
                                                        <label for={item.color} style={{ backgroundColor: item.color, border: '1px solid rgba(0,0,0,0.2)' }}></label>
                                                    </div>)}
                                                </div>
                                            </Accordion.Body>
                                        </Accordion.Item>
                                    </Accordion>
                                </div>
                                <div className='pt-4 pb-3 border-bottom border-tertiary'>
                                    <Accordion defaultActiveKey="0" id='size-options'>
                                        <Accordion.Item eventKey="0" className='border-0'>
                                            <Accordion.Header><h5 className='fw-semibold'>Size</h5></Accordion.Header>
                                            <Accordion.Body className='py-3 px-0'>
                                                <div className={shopStyles.sizebtncontainer}>
                                                    {sizeVariations.map((item, index) => <div key={index}>
                                                        <button className={sizebtn === index ? shopStyles.activesizebtn : shopStyles.sizebtn} onClick={() => setSizeBtn(index)}>{item.size}</button>
                                                    </div>)}
                                                </div>
                                            </Accordion.Body>
                                        </Accordion.Item>
                                    </Accordion>
                                </div>
                                <div className='pt-4 pb-3'>
                                    <Accordion defaultActiveKey="0" id='dress-style'>
                                        <Accordion.Item eventKey="0" className='border-0'>
                                            <Accordion.Header><h5 className='fw-semibold'>Dress Style</h5></Accordion.Header>
                                            <Accordion.Body className='py-3 px-0'>
                                                {dressStyle.map((item, index) =>
                                                    <ul className='list-unstyled' key={index}>
                                                        <div className='d-flex justify-content-between'>
                                                            <li><a href="#" className='text-decoration-none text-muted'>{item.style}</a></li>
                                                            <span><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                <path d="M6.53073 2.4694L11.5307 7.4694C11.6007 7.53908 11.6561 7.62187 11.694 7.71304C11.7318 7.8042 11.7513 7.90194 11.7513 8.00065C11.7513 8.09936 11.7318 8.1971 11.694 8.28827C11.6561 8.37943 11.6007 8.46222 11.5307 8.5319L6.53073 13.5319C6.38984 13.6728 6.19874 13.752 5.99948 13.752C5.80023 13.752 5.60913 13.6728 5.46823 13.5319C5.32734 13.391 5.24818 13.1999 5.24818 13.0007C5.24818 12.8014 5.32734 12.6103 5.46823 12.4694L9.93761 8.00003L5.46761 3.53065C5.32671 3.38976 5.24756 3.19866 5.24756 2.9994C5.24756 2.80015 5.32671 2.60905 5.46761 2.46815C5.60851 2.32726 5.7996 2.2481 5.99886 2.2481C6.19812 2.2481 6.38921 2.32726 6.53011 2.46815L6.53073 2.4694Z" fill="black" fill-opacity="0.6" />
                                                            </svg>
                                                            </span>
                                                        </div>
                                                    </ul>
                                                )}
                                            </Accordion.Body>
                                        </Accordion.Item>
                                    </Accordion>
                                </div>
                                <div>
                                    <button className={`${shopStyles.applyfilterbtn} d-block w-100`}>Apply Filter</button>
                                </div>
                            </div>
                        </Offcanvas.Body>
                    </Offcanvas>
                </Row>
            </Container>
        </section>
    )
}

export default Shop